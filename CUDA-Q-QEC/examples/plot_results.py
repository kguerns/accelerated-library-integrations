...
def write_summary(rows, output_dir):
    ...
    surface_rows = [
        ...
        and row.get("decoder") == "nv-qldpc-decoder"
    ...
    if surface_rows:
        if all(int(row["rounds"]) == int(row["distance"]) for row in surface_rows):
            surface_note = (
                "This circuit-level memory sweep uses rounds=d, lower physical error rates, "
                "and QLDPC decoding. It is the realistic integration demo."
            )
        else:
            surface_note = (
